...
static void buildkey0list(void)
...
uword	temp;
    ...
    if (i < 13) {
        /* One of the LSBs didn't match => false hit */
        return;
    }
...
byte	diff;
     ...
     *
     * It can easily be verified that for any two bytes b1 > 0, b2:
     * MSB( b1*INVCONST - b2 ) = MSB( b1*INVCONST )
     * (simple exhaustive test on 2^16 - 2^8 combinations)
     * and for b1 = 0, b2 > 0
     * MSB( b1*INVCONST - b2 ) = 0xff
     * (obvious)
     * and for b1 = 0, b2 = 0
     * MSB( b1*INVCONST - b2 ) = 0
     * (obvious)
     *
     * mTab2 has been tweaked for the special case of b1 = 0, so
     * here we can simply drop LSB(key0_{n-1}) from
     * MSB(LSB(key0_n)*INVCONST - LSB(key0_{n-1})) in (a) and (b).
     *
...
static void buildkey1list(void)
...
ushort  k;
uword	key2j, key2iminus1, key2iminus2, newKey, oldValue = 0;
	...
	if( (tempTable[key3iminus1]
		      [invTempTable[key3iminus1][(key2iminus1&0xfc00)>>10]
				   [k]]&0xff00) == (ushort) (key2iminus1&0xff00))
	{
	    newKey = key2iminus1 |
